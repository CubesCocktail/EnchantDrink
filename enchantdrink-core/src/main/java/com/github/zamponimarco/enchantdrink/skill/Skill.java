...
public abstract class Skill implements Model, Cloneable, Keyed {
    ...
    public Skill(ModelPath<Enchant> path) {
        this(path.getRoot().getId(), UUID.randomUUID(), new ArrayList<>(DEFAULT_SLOTS),
                Lists.newArrayList());
    }

    public Skill(UUID enchantId, UUID id, List<Slot> allowedSlots, List<ActionGroup> groups) {
    ...
    public Skill(Map<String, Object> map) {
        this.enchantId = UUID.fromString((String) map.getOrDefault("enchantId", UUID.randomUUID().toString()));
    ...
    @Override
    public Key getKey() {
        return new SkillKey(enchantId, id);
    }
